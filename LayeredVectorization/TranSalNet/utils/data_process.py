...
def preprocess_img(imgage_input, channels=3):

    # Check if input is a file path or numpy array
    if isinstance(imgage_input, str):
        # Original behavior: load from file
        if channels == 1:
            img = cv2.imread(imgage_input, 0)
        elif channels == 3:
            img = cv2.imread(imgage_input)
    else:
        # New behavior: use numpy array directly
        img = imgage_input
...
def postprocess_img(pred, org_input):
    pred = np.array(pred)
    # Check if input is a file path or numpy array
    if isinstance(org_input, str):
        # Original behavior: load from file
        org = cv2.imread(org_input, 0)
    else:
        # New behavior: use numpy array directly
        org = org_input
